...
\begin{itemize}
    ...
    \item \textbf{Combine}: Average training time is $< 3$ minutes on a single NVIDIA RTX 4090 GPU, demonstrating extreme computational efficiency.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Stationarity}: KPSS Test rejects the null hypothesis of stationarity (p-value = 0.0102), confirming that the upward trend in $\tau$ is structural.
...
\begin{equation}
    \int_{-\infty}^{\infty} e^x u(T, x) \, dx = e^{rT}
\end{equation}
This integral constraint allows us to enforce the no-arbitrage condition directly within the PINN loss function without making assumptions about the specific form of the stochastic process (e.g., GBM or Jump Diffusion).
...
Batch Size & 5000 (Collocation Points) \\
Epochs & 5000 \\
$\lambda_{PDE}$ (Physics Weight) & 1.0 \\
$\lambda_{IC}$ (Initial Cond. Weight) & 10.0 \\
